...
class diguinet(threading.Thread):
    ...
    def run(self):
        ...
        images256_ = tf.placeholder('float', shape=[None, 256, 256, 3])
        images256 = images256_ / 128.0 - 1.0
        labels = tf.placeholder('float', [None, 256, 256, 3])
        masks_ = tf.placeholder('float', [None, 256, 256])

        self.build(images256,train=False,num_classes=2,random_init_fc8=False)

        labels_elaster = (tf.log(labels[:, :, :, 0]) / tf.log(5.0))
        labels_poisson = tf.maximum(2 * labels[:, :, :, 1], 1.0 / 3 * labels[:, :, :, 1])
        loss_elaster = tf.square(labels_elaster - self.upscore32[:, :, :, 0])
        loss_poisson = tf.square(labels_poisson - self.upscore32[:, :, :, 1])
        ...
        if cfg.IS_RESTORE:
            saver.restore(sess, './weight/lixue_full_cov.cktp-' + str(cfg.SAVED_WEIGHT_STEPS))
            print('restore '+'./weight/lixue_full_cov.cktp-' + str(cfg.SAVED_WEIGHT_STEPS))
        else:
        ...
        for i in range(1, cfg.EPOCHS):
            ...
            if (i-1)%cfg.LOSS_PRINT_STEPS == 0:
                sunshi = sess.run([loss_el,loss_po],feed_dict={images256_: batch[0], labels: batch[1][:, :, :, 0:3],masks_: batch[1][:, :, :, 3]})
                loss_info = 'EPOCHS' + str(i) + ':' + str(sunshi)
                self.log_file.write(loss_info+'\n')
                print(loss_info)

            sess.run([train_step_el,train_step_po],
                     feed_dict={images256_: batch[0], labels: batch[1][:, :, :, 0:3], masks_: batch[1][:, :, :, 3]})

            if i%cfg.SAVE_STEPS == 0 or self.__save.isSet():
                
                saver.save(sess, './weight/lixue_full_cov.cktp', global_step=i)
    ...
    def _fc_layer(self, bottom, name, num_classes=None,
            ...
            with tf.variable_scope(name) as scope:
                ...
                if name == 'fc6':
                    filt = self.get_fc_weight_reshape(name, [8, 8, 512, 4096])
                elif name == 'score_fr':
